...
class BoxMatchesController extends Controller
{
     ...
     * @var string
     */
    protected $title = 'Box Match Conflict';
    ...
    {
        return $content
            ->title("View Box Matches With Conflicts")
            ->description("show conflicted box fights")
            ->body($this->detail($id))
    ...
    protected function grid()
    {
        ...
        $grid->model()->where('conflict_flag', 2);
        return $grid;
    }
    ...
    protected function detail($id)
    {
        ...
        $show->field('conflict_flag', __('Conflict flag'));
        ...
        $show->field('status', __('Status'))->replace([1 => 'Joining in Progress', 2 => 'Match is Full', 3 => 'Match is Live', 4 => 'Match Ended', 5 => 'Match Finished', 6 => 'Match Cancelled']);
        ...
        $show->divider();
        $show->divider();
        $show->boxMatchUsers('Box Match Participants with Proofs', function ($comments) {
            ...
            $comments->user_id();
            $comments->username();
            $comments->column('team_id', 'Team')->sortable();
            $comments->is_host()->replace([null => 'No', 1 => 'Yes']);
            $comments->result();
            $comments->proof_image()->display(function ($pictures) {

                return json_decode($pictures, true);

            })->downloadable();
            // ->image('https://www.google.com/', 400, 400);

            $comments->proof_video()->display(function ($pictures) {
    ...
    {
        $deleteMatch = new \Encore\Admin\Widgets\Form;
        $deleteMatch->title = 'Delete Box Fight';
        $deleteMatch->action("/admin/box-match/delete-match-conflict/$id");
        $deleteMatch->method('GET');
        $deleteMatch->html('Delete the box fight');
        $deleteMatch->confirm('Are you sure?');
        $deleteMatch->disableReset();

        $winnerTeam = new \Encore\Admin\Widgets\Form;
        $winnerTeam->title = 'Select winner team for box fight ';
        $winnerTeam->action("/admin/box-match/conflict-Resolve/$id");
        $winnerTeam->method('POST');
        $winnerTeam->html('Use "0" to cancel the match');
        $winnerTeam->number('status', 'Winner Team?')->min('0')->max(BoxmatchUser::where('match_id', $id)->distinct()->count('team_id'))->default('0');
        $winnerTeam->confirm('Are you sure?');
        $winnerTeam->disableReset();

        $tabs = (new Tab)
        ->add('Result', $winnerTeam->render())
        ->add('Delete', $deleteMatch->render())
        ->render();
    ...
    public function adminResult(request $request, $match_id)
    {
        if ($request->status != BoxMatches::WINNER_TEAM1 && $request->status != BoxMatches::WINNER_TEAM2 && $request->status != BoxMatches::WINNER_CANCELLED) {
        ...
        $data = BoxMatches::find($match_id);
        if($data->status != 4){
            return response()->json(
                [
                    'message' => 'Match not active anymore',
                ],
                400
            );
        }

        $teamMembers = BoxmatchUser::where('match_id', $match_id)->get();
        $award = $data->bid_amount * 2;
        if ($request->status == BoxMatches::WINNER_TEAM1) {
            ...
            BoxMatches::finishNotification($data);
        }
        if ($request->status == BoxMatches::WINNER_TEAM2) {
            foreach ($teamMembers as $teamMember) {
                if ($teamMember->team_id == 2) {
                    BoxMatches::teamMemberWon($teamMember, $award, $data);
                }
        ...
        if ($request->status == BoxMatches::WINNER_CANCELLED) {
            foreach ($teamMembers as $teamMember) {
        ...
        return redirect("/admin/box-matches-conflict");
    }
